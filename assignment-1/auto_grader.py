...
subdirectories = ['first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eighth', 'ninth', 'tenth']
formats = {'first':'line', 'second':'file', 'third':'file', 'fourth':'file', 'fifth':'file', 'sixth':'line', 'seventh':'line', 'eighth':'file', 'ninth':'file', 'tenth':'file'}# if a program has single liner input and output, we put all test cases in single file. Otherwise, we have a file for test and associated file with results
weight = {'first':0.25, 'second':0.5, 'third':1.25, 'fourth':0.9375, 'fifth':1.875, 'sixth':0.25, 'seventh':0.25, 'eighth':1.5, 'ninth':2.5}
scores = {'first':0, 'second':0, 'third':0, 'fourth':0, 'fifth':0, 'sixth':0, 'seventh':0, 'eighth':0, 'ninth':0}
...
def file_grade(dirname):
    ...
    for testfile in sorted(os.listdir(test_dir)):
	    ...
	    #ret = "blah"
            ...
            ret = run_command("./%s %s"%(dirname, test_dir + testfile), user_program=True);
        except:
...
if __name__ == '__main__':
    ...
    test_cases_directory = os.getcwd() + "/testcases/grading_testcases/"
    if len(sys.argv)>1:
