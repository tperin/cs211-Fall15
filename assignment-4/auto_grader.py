...
def file_grade(dirname):
	...
	if not os.path.isfile(test_dir + testfile1):
		print "Found one of the test files, %s, but could not find the other"%(testfile)
		continue
        resultfile = "out"+testfile[4:len(testfile)]
        if not os.path.isfile(test_dir + resultfile):
            print "Found test files, %s and %s, but no output file."%(testfile,testfile1)
            continue
        print "Found both test files: %s and %s. The output will be compared to the file %s"%(testfile,testfile1,resultfile)
        try:
	    print "Now testing " + dirname + " on " + testfile + " and " + testfile1
            run_command("./%s %s %s >> %s"%(dirname, test_dir + testfile, test_dir + testfile1, resultfile), timeout=runtime[dirname], user_program=True);
		...
		print "Your output is not correct\n"
            ...
            print "An exception occurred while executing %s on %s and %s\n"%(dirname, testfile, testfile1)
            #os.chdir(prevdir)
            #print "Score is ", score
            #return
	#time.sleep(10)
...
if __name__ == '__main__':
    ...
    print "Your TOTAL SCORE = ", Total_score, "/99.0 for the public test cases"
